...
| Preuve produit       | Playground, débit de crédits, KIE et Stripe/FedaPay sont implémentés ; adoption et disponibilité réelle non prouvées                                                               | `src/routes/app.models.$slug.tsx`, `src/lib/api/generate.ts`, `src/lib/api/payments.ts` | Partiellement observée, staging à confirmer                    | Engineering                  | Exécuter un smoke test de bout en bout avec données bornées                     | Les anciens documents disent encore “mock” et doivent être réconciliés                      |
...
| Release / opérations | Runbook, checklist de cutover et callbacks documentés ; route de migration legacy à retirer ; observabilité à renforcer                                                            | `PRODUCTION-RUNBOOK.md`, `LIVE-CUTOVER-CHECKLIST.md`, `POST-LAUNCH-BACKLOG.md`          | `blocked` pour release publique tant que le P0 n’est pas fermé | Engineering                  | Supprimer la route GET de migration, puis lancer les gates build/test/typecheck | Risques : migration publique, variables d’environnement, callbacks inline                   |
...
Supprimer le chemin public `/run-migration`, conserver les migrations versionnées comme seule voie opérateur, exécuter `pnpm test`, `pnpm exec tsc --noEmit` et `pnpm exec vite build`, puis créer la preuve staging du paiement Mobile Money et du paiement carte avec replay idempotent.
