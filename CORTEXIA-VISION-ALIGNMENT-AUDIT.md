...
| Crypto and Alipay                     | UI advertises both; crypto is routed through Stripe as `card`, while Alipay is selected as `alipay`.                                                      | **Crypto label is incorrect; Alipay availability unverified** | **P1**   | High        |
...
This may be an intentional launch scope, but the product does not communicate it as such. The current copy describes an all-models platform while the actual creator-facing surface is primarily image/video. The correct fix is not necessarily to expose every unfinished category immediately; it is to choose one truthful position: either open the categories to creators with verified endpoints, or market the current release as an image/video launch with additional modalities in controlled beta.
...
The account UI advertises Mobile Money, card, crypto, and Alipay. Mobile Money is conditionally hidden when the public FedaPay key is absent. Card, crypto, and Alipay are routed into the same Stripe checkout function. The server maps `method === "alipay"` to Stripe’s `alipay` payment method and maps every other method—including `crypto`—to `card`. Therefore the UI currently labels a card checkout as crypto. This is not merely a missing integration; it is a mismatch between the purchase choice shown to the user and the payment method actually requested from Stripe.

“Payable partout” is also too broad for the evidence available. The repository supports one mobile-money provider and Stripe-based methods, but no local evidence proves universal regional coverage, availability of every displayed method, or successful live-money completion. The public claim should be narrowed or replaced with a precise payment availability statement.
...
| Pay by crypto             | Select crypto and actually use a crypto payment method.               | UI selection is mapped to Stripe `card`.                                                                                  | **Incorrect / misleading.**                           |
