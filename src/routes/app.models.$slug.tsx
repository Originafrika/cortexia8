...
import { ArrowLeft, Play, Sparkles, RefreshCw, Download, AlertTriangle, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/app/models/$slug")({
  ...
  errorComponent: ({ error }) => (
    <div className="mx-auto max-w-2xl px-6 py-24 text-center">
      ...
      <Link to="/app/models" className="mt-6 inline-flex text-amber-soft hover:underline">Retour au catalogue</Link>
    </div>
  ),
  notFoundComponent: () => (
    <div className="mx-auto max-w-2xl px-6 py-24 text-center">
...
function ModelPlayground() {
  ...
  function generate() {
    ...
    const duration = model.category === "video" ? 4200 : model.category === "audio" ? 2600 : model.category === "text" ? 1800 : 2200;
    const steps = 40;
    ...
    timers.current.push(window.setTimeout(() => {
      ...
      setResult({
        ...
        ratio: (state.ratio as string) === "9:16" ? "aspect-[9/16]" : (state.ratio as string) === "16:9" ? "aspect-video" : (state.ratio as string) === "3:4" ? "aspect-[3/4]" : (state.ratio as string) === "4:3" ? "aspect-[4/3]" : "aspect-square",
      });
    }, duration + 60));
  }
    ...
    <div className="mx-auto max-w-6xl px-5 sm:px-8 py-8">
      ...
      <div className="mt-4 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4 sm:flex sm:flex-wrap sm:justify-between">
        <div className="min-w-0">
          ...
          <h1 className="mt-2 font-display text-3xl sm:text-4xl tracking-[-0.03em] truncate">{model.name}</h1>
          <p className="mt-1 text-muted-foreground max-w-xl">{model.blurb}</p>
        </div>
        <div className="text-right shrink-0">
          <PriceDisplay usd={currentPrice} className="font-display text-2xl sm:text-3xl tracking-[-0.02em]" emphasize />
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1.15fr,0.85fr]">
        <div className="surface-gradient-border rounded-2xl bg-surface-1/60 p-6">
          ...
          <div className="space-y-5">
            {simple.map((p: ParamSpec, i: number) => (
              <Field key={i} p={p} state={state} setState={setState} prompt={prompt} setPrompt={setPrompt} />
            ))}
            <motion.div
              initial={false}
          ...
          <button
            ...
            className="mt-6 w-full inline-flex items-center justify-center gap-2 rounded-xl bg-amber px-5 py-3 text-sm font-medium text-primary-foreground hover:opacity-95 transition disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {status === "loading" ? (
        ...
        <div>
          <ResultPanel status={status} progress={progress} error={error} result={result} model={model} onRetry={generate} onReset={() => { setStatus("idle"); setResult(null); setError(null); }} />
        </div>
      </div>
    </div>
  );
...
function ResultPanel({
  status, progress, error, result, model, onRetry, onReset,
}: {
  status: Status; progress: number; error: string | null; result: Result | null; model: Model;
  onRetry: () => void; onReset: () => void;
}) {
  const ratioClass = result?.ratio ?? "aspect-square";
  return (
    <div className="space-y-4">
      <div className={"surface-gradient-border rounded-2xl bg-surface-1/60 overflow-hidden relative grid place-items-center " + ratioClass}>
        <AnimatePresence mode="wait">
              ...
              <div className="absolute inset-0 grid place-items-center text-center px-6">
                <div>
                  ...
                  <div className="mt-3 font-mono text-[10px] uppercase tracking-[0.22em] text-muted-foreground">
                    {model.category === "video" ? "Rendu vidéo" : model.category === "audio" ? "Synthèse vocale" : model.category === "text" ? "Rédaction" : "Rendu image"}
                  </div>
                  ...
                  <div className="mt-3 mx-auto w-40 h-1 rounded-full bg-surface-3 overflow-hidden">
                    <div className="h-full bg-amber transition-[width] duration-150" style={{ width: `${progress}%` }} />
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          {status === "error" && (
            <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center px-6">
              <AlertTriangle className="size-6 mx-auto text-amber" />
              <div className="mt-3 font-mono text-[10px] uppercase tracking-[0.22em] text-amber-soft">Échec</div>
              <div className="mt-1 text-sm text-foreground/80 max-w-xs mx-auto">{error}</div>
              <button onClick={onRetry} className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-amber/40 bg-amber/10 px-3 py-1.5 text-xs text-amber-soft hover:bg-amber/20 transition">
                <RefreshCw className="size-3.5" /> Réessayer
              </button>
            </motion.div>
          )}
          {status === "success" && result && (
            <motion.div key="success" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="absolute inset-0"
              style={{ background: `linear-gradient(135deg, ${result.tint}, oklch(0.14 0 0))` }}>
              <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between">
                <div className="rounded-full bg-black/60 backdrop-blur px-2 py-1 text-[10px] font-mono uppercase tracking-wider">{model.category}</div>
                <div className="rounded-full bg-black/60 backdrop-blur px-2 py-1"><PriceDisplay usd={result.cost} className="text-[10px]" /></div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {status === "success" && result && (
        <div className="surface-gradient-border rounded-2xl bg-surface-1/60 p-4 space-y-3">
          <div>
            <div className="text-[10px] font-mono uppercase tracking-[0.22em] text-muted-foreground">Prompt</div>
            <div className="mt-1 text-sm text-foreground/90 line-clamp-3">{result.prompt}</div>
          </div>
          <div className="flex gap-2">
            <button onClick={onRetry} className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl border border-border bg-surface-2/40 px-3 py-2 text-xs hover:border-amber/40 transition">
              <RefreshCw className="size-3.5" /> Régénérer
            </button>
            <button className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-amber text-primary-foreground px-3 py-2 text-xs font-medium hover:opacity-95 transition">
              <Download className="size-3.5" /> Télécharger
            </button>
          </div>
          <button onClick={onReset} className="w-full text-[11px] text-muted-foreground hover:text-foreground">Nouvelle génération</button>
        </div>
      )}
...
function Field({
  p, state, setState, prompt, setPrompt,
}: {
  p: ParamSpec;
