      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
            ...
            <a href="${magicLink}" style="display: inline-block; background: #f59e0b; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 500;">Access Cortexia</a>
...
export const sendAllMagicLinks = createServerFn({ method: "POST" })
  ...
  .handler(async () => {
    const rows = (await sql`SELECT email FROM waitlist ORDER BY created_at ASC`) as {
      email: string;
    }[];
...
export const sendMagicLink = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const exists = (await sql`SELECT id FROM waitlist WHERE email = ${data.email}`) as {
      id: number;
    }[];
