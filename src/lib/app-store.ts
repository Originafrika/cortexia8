...
export const useAppStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      addGeneration: (gen) =>
        set((state) => ({ activeGens: { ...state.activeGens, [gen.id]: gen } })),
      updateGeneration: (id, updates) =>
        set((state) => ({
          activeGens: { ...state.activeGens, [id]: { ...state.activeGens[id], ...updates } },
        })),
      removeGeneration: (id) =>
        set((state) => {
          const { [id]: _, ...rest } = state.activeGens;
          return { activeGens: rest };
        }),
      addToHistory: (entry) =>
        set((state) => ({ history: [entry, ...state.history].slice(0, 100) })),
      clearHistory: () => set({ history: [] }),
      setCanvasState: (nodes, edges, workflowId) =>
        set({ canvasNodes: nodes, canvasEdges: edges, canvasWorkflowId: workflowId }),
      setPlaygroundState: (prompt, params, modelSlug) =>
        set({ playgroundPrompt: prompt, playgroundParams: params, lastModelSlug: modelSlug }),
  ...
  for (const [id, gen] of Object.entries(activeGens)) {
    ...
    try {
      const status = await generationStatus({
        data: { id: gen.runId, sessionToken: session.token },
      });
      if (
        status.status === "success" ||
        status.status === "succeeded" ||
        status.nodes?.[0]?.status === "success" ||
        ...
        status.nodes?.[0]?.status === "error" ||
        status.nodes?.[0]?.status === "failed"
      ) {
        updateGeneration(id, { status: "error", error: "Generation failed" });
      }
    } catch (err) {
      console.error(
        `[app-store] verifyActiveGens poll failed: id=${id}, runId=${gen.runId}, model=${gen.modelSlug}`,
        err,
      );
    }
  }
